...
function validatePath(api, path, pathId, operationIds) {
  for (let operationName of swaggerMethods) {
    ...
    if (operation) {
      ...
      if (summary) {
        var isUpper = summary[0] === summary[0].toUpperCase();
        if (!isUpper || summary.substr(summary.length - 1) != ".") {
          throw ono.syntax(`Validation failed. Summary should start with an uppercase letter and end with a '.' at '${operationName}' in '${pathId}'`);
        }
      } else {
        throw ono.syntax(`Validation failed. Summary for method '${operationName}' in '${pathId}' must be present`)
      }
      if (declaredOperationId) {
        ...
        throw ono.syntax(`Validation failed. Operation id for method '${operationName}' in '${pathId}' must be present`)
      }
      ...
      for (let response of responses) {
        if (response >= 200 && response < 300) {
          validReponses.push(response);
        }
      }
      if (validReponses.length === 0) {
        throw ono.syntax(`Validation failed. Operation for method '${operationName}' in '${pathId}' must have atleast one 2xx response`)
      }
...
function validateSchema(schema, schemaId, validTypes) {
  if (validTypes.indexOf(schema.type) === -1 && validTypes.indexOf(schema.schema.type) === -1) {
    throw ono.syntax(
